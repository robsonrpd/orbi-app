...
import { obterBroadcastAtivo, listarHistoricoBroadcasts, listarOrigensDeContatos, obterAquecimentoWhatsApp } from '@/lib/actions/broadcast'
import { EnvioMassaClient } from './envio-massa-client'

export default async function EnvioMassaPage() {
  await guardModo('envio_massa')

  const [ativo, historico, origens, aquecimento] = await Promise.all([
    obterBroadcastAtivo(),
    listarHistoricoBroadcasts(),
    listarOrigensDeContatos(),
    obterAquecimentoWhatsApp(),
  ])
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden bg-[#F0F2F5]">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <EnvioMassaClient ativoInicial={ativo} historicoInicial={historico} origens={origens} aquecimento={aquecimento} />
      </div>
    </div>
  )
}
